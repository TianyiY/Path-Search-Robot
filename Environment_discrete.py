...
class Robot(object):
    ...
    action_N = 4 # (orientations - E, N, W, S)

    def __init__(self):
        self.robot_info = np.zeros(1, dtype=[('x', np.float16), ('y', np.float16), ('l', np.float16)])
        self.robot_info['x'] = 444    # robot center location x coordinate
        self.robot_info['y'] = 794    # robot center location y coordinate
        self.robot_info['l'] = 10    # robot length
        self.within_target=0
        self.action_info=[0., 0.5*np.pi, 1.*np.pi, 1.5*np.pi]
    ...
    def step(self, action_index):
        ...
        dx=np.cos(self.action_info[action_index]) * self.refresh_frequency    # calculate step length in x
        dy=np.sin(self.action_info[action_index]) * self.refresh_frequency    # calculate step length in y
    ...
    def random_choose_action(self):
        action_index=np.random.randint(0, self.action_N)    # sample action
        return action_index
